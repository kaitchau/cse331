...
# Crack Salted passwords with Rainbow Table
...
def salty_rainbow_table(salts):
    ...
    for salt in salts:
        for password in common_passwords:
            salty_pw = password+salt
            hashed_common_passwords[hashlib.md5(salty_pw.encode()).hexdigest()] = password
    return hashed_common_passwords

def salty_rainbow_attack(data,rainbow):
...
if __name__ == "__main__":
    ...
    salts = []
    for _,_,salt in pw_db:
        salts.append(salt)
    rainbow =  salty_rainbow_table(salts)
    salty_rainbow_attack(pw_db, rainbow)